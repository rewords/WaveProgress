package net.tygaware.waveprogress.waveprogresslib;
...
public class WaveProgress extends View {
        ...
        private void init(Context context, AttributeSet attrs, int defStyleAttr) {
            ...
            // Init ShapeType
            mShapeType = attributes.getInteger(R.styleable.WaveProgress_shape_type, DEFAULT_WAVE_SHAPE);

            // Init Wave
            mWaveColor = attributes.getColor(R.styleable.WaveProgress_wave_color, DEFAULT_WAVE_COLOR);

            // Init AmplitudeRatio
            float amplitudeRatioAttr = attributes.getFloat(R.styleable.WaveProgress_wave_amplitude, DEFAULT_AMPLITUDE_RATIO) / 1000;
            mAmplitudeRatio = (amplitudeRatioAttr > DEFAULT_AMPLITUDE_RATIO) ? DEFAULT_AMPLITUDE_RATIO : amplitudeRatioAttr;

            // Init Progress
            int progressMax = attributes.getInteger(R.styleable.WaveProgress_progress_max, DEFAULT_WAVE_PROGRESS_MAX_VALUE);
            setProgressMaxValue(progressMax);
            int progressValue = attributes.getInteger(R.styleable.WaveProgress_progress, DEFAULT_WAVE_PROGRESS_VALUE);
            setProgressValue(progressValue);

            //Init Animation Timing
            int progressAnim = attributes.getInteger(R.styleable.WaveProgress_progress_anim_duration, DEFAULT_PROGRESS_ANIMATION_DURATION);
            ...
            mBorderPaint.setStrokeWidth(attributes.getDimension(R.styleable.WaveProgress_border_width, dp2px(DEFAULT_BORDER_WIDTH)));
            mBorderPaint.setColor(attributes.getColor(R.styleable.WaveProgress_border_color, DEFAULT_WAVE_COLOR));

            // Init Title
            mTopTitlePaint = new Paint();
            mTopTitlePaint.setColor(attributes.getColor(R.styleable.WaveProgress_title_top_color, DEFAULT_TITLE_COLOR));
            mTopTitlePaint.setStyle(Paint.Style.FILL);
            mTopTitlePaint.setAntiAlias(true);
            mTopTitlePaint.setTextSize(attributes.getDimension(R.styleable.WaveProgress_title_top_size, sp2px(DEFAULT_TITLE_TOP_SIZE)));
            mTopTitle = attributes.getString(R.styleable.WaveProgress_title_top);

            mCenterTitlePaint = new Paint();
            mCenterTitlePaint.setColor(attributes.getColor(R.styleable.WaveProgress_title_center_color, DEFAULT_TITLE_COLOR));
            mCenterTitlePaint.setStyle(Paint.Style.FILL);
            mCenterTitlePaint.setAntiAlias(true);
            mCenterTitlePaint.setTextSize(attributes.getDimension(R.styleable.WaveProgress_title_center_size, sp2px(DEFAULT_TITLE_CENTER_SIZE)));
            mCenterTitle = attributes.getString(R.styleable.WaveProgress_title_center);

            mBottomTitlePaint = new Paint();
            mBottomTitlePaint.setColor(attributes.getColor(R.styleable.WaveProgress_title_bottom_color, DEFAULT_TITLE_COLOR));
            mBottomTitlePaint.setStyle(Paint.Style.FILL);
            mBottomTitlePaint.setAntiAlias(true);
            mBottomTitlePaint.setTextSize(attributes.getDimension(R.styleable.WaveProgress_title_bottom_size, sp2px(DEFAULT_TITLE_BOTTOM_SIZE)));
            mBottomTitle = attributes.getString(R.styleable.WaveProgress_title_bottom);
        }
